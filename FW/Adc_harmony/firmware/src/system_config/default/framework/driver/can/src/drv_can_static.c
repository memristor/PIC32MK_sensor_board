...
void DRV_CAN0_Initialize(void)
{
    ...
    PLIB_CAN_Enable(CAN_ID_4);

    /* Switch the CAN module to Configuration mode. Wait until the switch is complete */
    PLIB_CAN_OperationModeSelect(CAN_ID_4, CAN_CONFIGURATION_MODE);
    while(PLIB_CAN_OperationModeGet(CAN_ID_4) != CAN_CONFIGURATION_MODE);

    PLIB_CAN_PhaseSegment2LengthFreelyProgrammableEnable(CAN_ID_4);

    //Set the Baud rate to 500 kbps
    PLIB_CAN_PropagationTimeSegmentSet(CAN_ID_4, 1-1);
    PLIB_CAN_PhaseSegment1LengthSet(CAN_ID_4, 3-1);
    PLIB_CAN_PhaseSegment2LengthSet(CAN_ID_4, 3-1);
    PLIB_CAN_SyncJumpWidthSet(CAN_ID_4, 1-1);
    PLIB_CAN_BaudRatePrescaleSet(CAN_ID_4, 14); // set to 1 higher then ECAN tool


    /* Assign the buffer area to the CAN module.
      In this case assign enough memory for 2
      channels, each with 8 message buffers.*/
    PLIB_CAN_MemoryBufferAssign(CAN_ID_4, can_message_buffer0);

    /* Configure CAN_ID_4 Channel for CAN_RX_FULL_RECEIVE operation. Allocate 8 message buffer, and assign low medium priority for transmissions. */
    PLIB_CAN_ChannelForReceiveSet(CAN_ID_4, CAN_CHANNEL0, 8, CAN_RX_FULL_RECEIVE);
    PLIB_CAN_FilterToChannelLink(CAN_ID_4, CAN_FILTER0, CAN_FILTER_MASK0, CAN_CHANNEL0);
    /* Configure CAN_ID_4 Channel for CAN_TX_RTR_DISABLED operation. Allocate 8 message buffer, and assign low medium priority for transmissions. */
    PLIB_CAN_ChannelForTransmitSet(CAN_ID_4, CAN_CHANNEL1, 8, CAN_TX_RTR_DISABLED, CAN_LOW_MEDIUM_PRIORITY);
    PLIB_CAN_FilterConfigure(CAN_ID_4, CAN_FILTER0, 0x7800, CAN_EID);
    PLIB_CAN_FilterEnable(CAN_ID_4, CAN_FILTER0);

    PLIB_CAN_FilterMaskConfigure(CAN_ID_4, CAN_FILTER_MASK0, 0xff00, CAN_EID, CAN_FILTER_MASK_IDE_TYPE);

    /* Switch the CAN module to Normal mode. Wait until the switch is complete */
    PLIB_CAN_OperationModeSelect(CAN_ID_4, CAN_NORMAL_MODE);
    while(PLIB_CAN_OperationModeGet(CAN_ID_4) != CAN_NORMAL_MODE);
...
void DRV_CAN0_Deinitialize(void)
{
    ...
    PLIB_CAN_OperationModeSelect(CAN_ID_4, CAN_DISABLE_MODE);
    while(PLIB_CAN_OperationModeGet(CAN_ID_4) != CAN_DISABLE_MODE);
...
bool DRV_CAN0_ChannelMessageTransmit(CAN_CHANNEL channelNum, int address, uint8_t DLC, uint8_t* message)
{
    ...
    int count = 0;
    if ( (PLIB_CAN_ChannelEventGet(CAN_ID_4, channelNum) & CAN_TX_CHANNEL_NOT_FULL) == CAN_TX_CHANNEL_NOT_FULL)
    {
        //drv_Message0 = (CAN_TX_MSG_BUFFER *) &can_message_buffer0[channelNum];
        drv_Message0 = PLIB_CAN_TransmitBufferGet(CAN_ID_4, channelNum);
       ...
       PLIB_CAN_ChannelUpdate(CAN_ID_4, channelNum);
       PLIB_CAN_TransmitChannelFlush(CAN_ID_4, channelNum);
        return(true);
    }
    return (false);
}
...
bool DRV_CAN0_ChannelMessageReceive(CAN_CHANNEL channelNum, int address, uint8_t DLC, uint8_t* message)
{
    ...
    ChannelEvent = PLIB_CAN_ChannelEventGet( CAN_ID_4 , channelNum );
    ...
    if( (ChannelEvent & CAN_RX_CHANNEL_NOT_EMPTY) == CAN_RX_CHANNEL_NOT_EMPTY )
    {
        ...
        RxMessage = (CAN_RX_MSG_BUFFER *)PLIB_CAN_ReceivedMessageGet(CAN_ID_4, channelNum);
